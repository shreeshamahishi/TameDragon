package org.tamedragon.codegen;

import org.tamedragon.common.llvmir.types.User;
import org.tamedragon.common.llvmir.types.Value;
import org.tamedragon.dagir.SelectionDAG;
import org.tamedragon.dagir.SelectionDagValue;
...
public class DAGConstructor {
	...
	private SelectionDAG selectionDAG;

	/// LimitFloatPrecision - Generate low-precision inline sequences for
	/// some float libcalls (6, 8 or 12 bits).
	static int LimitFloatPrecision;
...
LimitFPPrecision("limit-float-precision",
	...
	/*static int MaxParallelChains = 64;

	private void visitBinary(User user, int OpCode) {
		  SelectionDagValue Op1 = getValue(user.getOperand(0));
		  SelectionDagValue Op2 = getValue(user.getOperand(1));
		  setValue(user, selectionDAG.getNode(OpCode,
		                           Op1.getValueType(), Op1, Op2));
		}
	
	 private void setValue(Value V, SelectionDagValue NewN) {
		    SDValue &N = NodeMap[V];
	...
	private SelectionDagValue getValue(Value V) {
	...
	*/
	...
	/*
	static SelectionDagValue getCopyFromPartsVector(SelectionDAG DAG, DebugLoc DL,
			SelectionDagValue Parts, int NumParts,
...
/// (ISD::AssertSext).
static SelectionDagValue getCopyFromParts(SelectionDAG DAG, DebugLoc DL,
                                SelectionDagValue Parts,
                                int NumParts, EVT PartVT, EVT ValueVT,
  ...
  if (NumParts > 1) {
    ...
    if (ValueVT.isInteger()) {
      ...
      int RoundParts = NumParts & (NumParts - 1) ?
        1 << Log2_32(NumParts) : NumParts;
      int RoundBits = PartBits * RoundParts;
      EVT RoundVT = RoundBits == ValueBits ?
        ValueVT : EVT::getIntegerVT(*DAG.getContext(), RoundBits);
      SelectionDagValue Lo, Hi;
...
/// (ISD::AssertSext).
static SelectionDagValue getCopyFromPartsVector(SelectionDAG &DAG, DebugLoc DL,
                                      const SelectionDagValue *Parts, int NumParts,
                                      EVT PartVT, EVT ValueVT) {
  assert(ValueVT.isVector() && "Not a vector value");
  assert(NumParts > 0 && "No parts to assemble!");
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  ...
  if (NumParts > 1) {
    ...
    if (NumIntermediates == NumParts) {
      ...
      assert(NumParts % NumIntermediates == 0 &&
             "Must expand into a divisible number of parts!");
      int Factor = NumParts / NumIntermediates;
...
static void getCopyToPartsVector(SelectionDAG &DAG, DebugLoc dl,
                                 SelectionDagValue Val, SelectionDagValue *Parts, int NumParts,
                                 EVT PartVT);
...
static void getCopyToParts(SelectionDAG &DAG, DebugLoc DL,
  ...
  if (NumParts & (NumParts - 1)) {
    ...
    int RoundParts = 1 << Log2_32(NumParts);
    int RoundBits = RoundParts * PartBits;
    int OddParts = NumParts - RoundParts;
    SelectionDagValue OddVal = DAG.getNode(ISD::SRL, DL, ValueVT, Val,
  ...
  for (int StepSize = NumParts; StepSize > 1; StepSize /= 2) {
    for (int i = 0; i < NumParts; i += StepSize) {
      ...
      SelectionDagValue &Part0 = Parts[i];
      SelectionDagValue &Part1 = Parts[i+StepSize/2];
...
static void getCopyToPartsVector(SelectionDAG &DAG, DebugLoc DL,
  ...
  if (NumParts == 1) {
    if (PartVT == ValueVT) {
      ...
      for (int i = 0, e = ValueVT.getVectorNumElements(); i != e; ++i)
        Ops.push_back(DAG.getNode(ISD::EXTRACT_VECTOR_ELT, DL,
                                  ElementVT, Val, DAG.getIntPtrConstant(i)));

      for (int i = ValueVT.getVectorNumElements(),
  ...
  EVT IntermediateVT, RegisterVT;
  int NumIntermediates;
  int NumRegs = TLI.getVectorTypeBreakdown(*DAG.getContext(), ValueVT,
                                                IntermediateVT,
                                                NumIntermediates, RegisterVT);
  int NumElements = ValueVT.getVectorNumElements();
...
namespace {
  ...
  struct RegsForValue {
    ...
    RegsForValue(const SmallVector<int, 4> &regs,
    ...
    RegsForValue(LLVMContext &Context, const TargetLowering &tli,
      ...
      for (int Value = 0, e = ValueVTs.size(); Value != e; ++Value) {
        EVT ValueVT = ValueVTs[Value];
        int NumRegs = tli.getNumRegisters(Context, ValueVT);
    ...
    SelectionDagValue getCopyFromRegs(SelectionDAG &DAG, FunctionLoweringInfo &FuncInfo,
                            DebugLoc dl,
                            SelectionDagValue &Chain, SelectionDagValue *Flag) const;
    ...
    void AddInlineAsmOperands(int Kind,
                              bool HasMatching, int MatchingIdx,
                              SelectionDAG &DAG,
...
void RegsForValue::AddInlineAsmOperands(int Code, bool HasMatching,
                                        int MatchingIdx,
                                        SelectionDAG &DAG,
                                        std::vector<SDValue> &Ops) const {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();

  int Flag = InlineAsm::getFlagWord(Code, Regs.size());
  if (HasMatching)
  ...
  SelectionDagValue Res = DAG.getTargetConstant(Flag, MVT::i32);
  Ops.push_back(Res);

  for (int Value = 0, Reg = 0, e = ValueVTs.size(); Value != e; ++Value) {
    int NumRegs = TLI.getNumRegisters(*DAG.getContext(), ValueVTs[Value]);
    EVT RegisterVT = RegVTs[Value];
    for (int i = 0; i != NumRegs; ++i) {
...
SDValue SelectionDAGBuilder::getRoot() {
  ...
  if (PendingLoads.size() == 1) {
    SelectionDagValue Root = PendingLoads[0];
    DAG.setRoot(Root);
    PendingLoads.clear();
    return Root;
  }
  ...
  SelectionDagValue Root = DAG.getNode(ISD::TokenFactor, getCurDebugLoc(), MVT::Other,
...
SDValue SelectionDAGBuilder::getControlRoot() {
  SelectionDagValue Root = DAG.getRoot();
...
void SelectionDAGBuilder::visit(int Opcode, const User &I) {
...
void SelectionDAGBuilder::resolveDanglingDebugInfo(const Value *V,
  ...
  if (DDI.getDI()) {
    ...
    int DbgSDNodeOrder = DDI.getSDNodeOrder();
...
/// getValue - Return an SelectionDagValue for the given Value.
SDValue SelectionDAGBuilder::getValue(const Value *V) {
  // If we already have an SelectionDagValue for this value, use it. It's important
  // to do this first, so that we don't create a CopyFromReg if we already
  // have a regular SDValue.
  SelectionDagValue &N = NodeMap[V];
  if (N.getNode()) return N;

  // If there's a virtual register allocated and initialized for this
  // value, use it.
  DenseMap<const Value *, int>::iterator It = FuncInfo.ValueMap.find(V);
  if (It != FuncInfo.ValueMap.end()) {
    int InReg = It->second;
    RegsForValue RFV(*DAG.getContext(), TLI, InReg, V->getType());
    SelectionDagValue Chain = DAG.getEntryNode();
    N = RFV.getCopyFromRegs(DAG, FuncInfo, getCurDebugLoc(), Chain, NULL);
    resolveDanglingDebugInfo(V, N);
    return N;
  }

  // Otherwise create a new SelectionDagValue and remember it.
  SelectionDagValue Val = getValueImpl(V);
  NodeMap[V] = Val;
  resolveDanglingDebugInfo(V, Val);
  return Val;
}

/// getNonRegisterValue - Return an SelectionDagValue for the given Value, but
/// don't look in FuncInfo.ValueMap for a virtual register.
SDValue SelectionDAGBuilder::getNonRegisterValue(const Value *V) {
  // If we already have an SelectionDagValue for this value, use it.
  SelectionDagValue &N = NodeMap[V];
  if (N.getNode()) return N;
...
/// Create an SelectionDagValue for the given value.
SDValue SelectionDAGBuilder::getValueImpl(const Value *V) {
  if (const Constant *C = dyn_cast<Constant>(V)) {
    ...
    if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(C)) {
      ...
      SelectionDagValue N1 = NodeMap[V];
    ...
    if (isa<ConstantStruct>(C) || isa<ConstantArray>(C)) {
      ...
      for (User::const_op_iterator OI = C->op_begin(), OE = C->op_end();
        ...
        for (int i = 0, e = Val->getNumValues(); i != e; ++i)
    ...
    int NumElements = VecTy->getNumElements();
  ...
  if (const Instruction *Inst = dyn_cast<Instruction>(V)) {
    int InReg = FuncInfo.InitializeRegForValue(Inst);
...
void SelectionDAGBuilder::visitRet(const ReturnInst &I) {
  SelectionDagValue Chain = getControlRoot();
  SmallVector<ISD::OutputArg, 8> Outs;
  SmallVector<SDValue, 8> OutVals;

  if (!FuncInfo.CanLowerReturn) {
    int DemoteReg = FuncInfo.DemoteRegister;
    ...
    SelectionDagValue RetPtr = DAG.getRegister(DemoteReg, PtrValueVTs[0]);
    SelectionDagValue RetOp = getValue(I.getOperand(0));
    ...
    for (int i = 0; i != NumValues; ++i) {
      SelectionDagValue Add = DAG.getNode(ISD::ADD, getCurDebugLoc(),
...
void SelectionDAGBuilder::CopyToExportRegsIfNeeded(const Value *V) {
  ...
  DenseMap<const Value *, int>::iterator VMI = FuncInfo.ValueMap.find(V);
...
void SelectionDAGBuilder::ExportFromCurrentBlock(const Value *V) {
  ...
  int Reg = FuncInfo.InitializeRegForValue(V);
...
void SelectionDAGBuilder::FindMergedConditions(const Value *Cond,
  ...
  if (!BOp || !(isa<BinaryOperator>(BOp) || isa<CmpInst>(BOp)) ||
      (int)BOp->getOpcode() != Opc || !BOp->hasOneUse() ||
...
void SelectionDAGBuilder::visitSwitchCase(CaseBlock &CB,
                                          MachineBasicBlock *SwitchBB) {
  SelectionDagValue Cond;
  SelectionDagValue CondLHS = getValue(CB.CmpLHS);
  ...
  if (CB.CmpMHS == NULL) {
    ...
    if (CB.CmpRHS == ConstantInt::getTrue(*DAG.getContext()) &&
      ...
      Cond = CondLHS;
             ...
             CB.CC == ISD::SETEQ) {
      SelectionDagValue True = DAG.getConstant(1, CondLHS.getValueType());
    ...
    SelectionDagValue CmpOp = getValue(CB.CmpMHS);
    ...
    if (cast<ConstantInt>(CB.CmpLHS)->isMinValue(true)) {
      ...
      SelectionDagValue SUB = DAG.getNode(ISD::SUB, dl,
  ...
  if (CB.TrueBB == NextBlock) {
    ...
    SelectionDagValue True = DAG.getConstant(1, Cond.getValueType());
    Cond = DAG.getNode(ISD::XOR, dl, Cond.getValueType(), Cond, True);
  }

  SelectionDagValue BrCond = DAG.getNode(ISD::BRCOND, dl,
...
void SelectionDAGBuilder::visitJumpTable(JumpTable &JT) {
  ...
  SelectionDagValue Index = DAG.getCopyFromReg(getControlRoot(), getCurDebugLoc(),
                                     JT.Reg, PTy);
  SelectionDagValue Table = DAG.getJumpTable(JT.JTI, PTy);
  SelectionDagValue BrJumpTable = DAG.getNode(ISD::BR_JT, getCurDebugLoc(),
...
void SelectionDAGBuilder::visitJumpTableHeader(JumpTable &JT,
  ...
  SelectionDagValue SwitchOp = getValue(JTH.SValue);
  EVT VT = SwitchOp.getValueType();
  SelectionDagValue Sub = DAG.getNode(ISD::SUB, getCurDebugLoc(), VT, SwitchOp,
  ...
  int JumpTableReg = FuncInfo.CreateReg(TLI.getPointerTy());
  SelectionDagValue CopyTo = DAG.getCopyToReg(getControlRoot(), getCurDebugLoc(),
  ...
  // case in the switch.
  SelectionDagValue CMP = DAG.getSetCC(getCurDebugLoc(),
  ...
  SelectionDagValue BrCond = DAG.getNode(ISD::BRCOND, getCurDebugLoc(),
...
void SelectionDAGBuilder::visitBitTestHeader(BitTestBlock &B,
  ...
  SelectionDagValue SwitchOp = getValue(B.SValue);
  EVT VT = SwitchOp.getValueType();
  SelectionDagValue Sub = DAG.getNode(ISD::SUB, getCurDebugLoc(), VT, SwitchOp,
                            DAG.getConstant(B.First, VT));

  // Check range
  SelectionDagValue RangeCmp = DAG.getSetCC(getCurDebugLoc(),
  ...
  if (!TLI.isTypeLegal(VT))
    ...
    for (int i = 0, e = B.Cases.size(); i != e; ++i)
  ...
  SelectionDagValue BrRange = DAG.getNode(ISD::BRCOND, getCurDebugLoc(),
...
void SelectionDAGBuilder::visitBitTestCase(BitTestBlock &BB,
                                           ...
                                           MachineBasicBlock *SwitchBB) {
  EVT VT = BB.RegVT;
  SelectionDagValue ShiftOp = DAG.getCopyFromReg(getControlRoot(), getCurDebugLoc(),
                                       Reg, VT);
  SelectionDagValue Cmp;
  int PopCount = CountPopulation_64(B.Mask);
  if (PopCount == 1) {
    ...
    // Make desired shift
    SelectionDagValue SwitchVal = DAG.getNode(ISD::SHL, getCurDebugLoc(), VT,
                                    DAG.getConstant(1, VT), ShiftOp);

    // Emit bit tests and jumps
    SelectionDagValue AndOp = DAG.getNode(ISD::AND, getCurDebugLoc(),
  ...
  SelectionDagValue BrAnd = DAG.getNode(ISD::BRCOND, getCurDebugLoc(),
                              MVT::Other, getControlRoot(),
...
void SelectionDAGBuilder::visitLandingPad(const LandingPadInst &LP) {
  ...
  SelectionDagValue Ops[2];
  Ops[0] = DAG.getRoot();
  SelectionDagValue Op1 = DAG.getNode(ISD::EXCEPTIONADDR, getCurDebugLoc(), VTs, Ops, 1);
  SelectionDagValue Chain = Op1.getValue(1);

  // Insert the EHSELECTION instruction.
  VTs = DAG.getVTList(TLI.getPointerTy(), MVT::Other);
  Ops[0] = Op1;
  Ops[1] = Chain;
  SelectionDagValue Op2 = DAG.getNode(ISD::EHSELECTION, getCurDebugLoc(), VTs, Ops, 2);
  Chain = Op2.getValue(1);
  Op2 = DAG.getSExtOrTrunc(Op2, getCurDebugLoc(), MVT::i32);

  Ops[0] = Op1;
  Ops[1] = Op2;
  SelectionDagValue Res = DAG.getNode(ISD::MERGE_VALUES, getCurDebugLoc(),
                            DAG.getVTList(&ValueVTs[0], ValueVTs.size()),
                            &Ops[0], 2);
  ...
  if (Size == 2 && CR.CaseBB == SwitchBB) {
    ...
    if (Small.Low == Small.High && Big.Low == Big.High && Small.BB == Big.BB) {
      ...
      if (BigValue.countPopulation() == SmallValue.countPopulation() + 1 &&
        ...
        SelectionDagValue CondLHS = getValue(SV);
        EVT VT = CondLHS.getValueType();
        DebugLoc DL = getCurDebugLoc();

        SelectionDagValue Or = DAG.getNode(ISD::OR, DL, VT, CondLHS,
                                 DAG.getConstant(CommonBit, VT));
        SelectionDagValue Cond = DAG.getSetCC(DL, MVT::i1,
        ...
        SelectionDagValue BrCond = DAG.getNode(ISD::BRCOND, DL, MVT::Other,
                                     getControlRoot(), Cond,
...
bool SelectionDAGBuilder::handleJTSwitchCase(CaseRec &CR,
  ...
  int JTEncoding = TLI.getJumpTableEncoding();
  int JTI = CurMF->getOrCreateJumpTableInfo(JTEncoding)
...
bool SelectionDAGBuilder::handleBTSplitSwitchCase(CaseRec& CR,
  ...
  // Size is the number of Cases represented by this range.
  int Size = CR.Range.second - CR.Range.first;
...
bool SelectionDAGBuilder::handleBitTestsSwitchCase(CaseRec& CR,
  ...
  DEBUG(dbgs() << "Cases:\n");
  for (int i = 0, e = CasesBits.size(); i!=e; ++i) {
...
void SelectionDAGBuilder::UpdateSplitBlock(MachineBasicBlock *First,
  ...
  // Update JTCases.
  for (int i = 0, e = JTCases.size(); i != e; ++i)
    if (JTCases[i].first.HeaderBB == First)
      JTCases[i].first.HeaderBB = Last;

  // Update BitTestCases.
  for (int i = 0, e = BitTestCases.size(); i != e; ++i)
...
void SelectionDAGBuilder::visitIndirectBr(const IndirectBrInst &I) {
  ...
  for (int i = 0, e = I.getNumSuccessors(); i != e; ++i)
    succs.push_back(I.getSuccessor(i));
  array_pod_sort(succs.begin(), succs.end());
  succs.erase(std::unique(succs.begin(), succs.end()), succs.end());
  for (int i = 0, e = succs.size(); i != e; ++i) {
...
void SelectionDAGBuilder::visitFSub(const User &I) {
  ...
  if (isa<Constant>(I.getOperand(0)) &&
    ...
    SelectionDagValue Op2 = getValue(I.getOperand(1));
...
void SelectionDAGBuilder::visitBinary(const User &I, int OpCode) {
  SelectionDagValue Op1 = getValue(I.getOperand(0));
  SelectionDagValue Op2 = getValue(I.getOperand(1));
  setValue(&I, DAG.getNode(OpCode, getCurDebugLoc(),
                           Op1.getValueType(), Op1, Op2));
}

void SelectionDAGBuilder::visitShift(const User &I, int Opcode) {
  SelectionDagValue Op1 = getValue(I.getOperand(0));
  SelectionDagValue Op2 = getValue(I.getOperand(1));
  ...
  if (!I.getType()->isVectorTy() && Op2.getValueType() != ShiftTy) {
    ...
    int Op2Size = Op2.getValueType().getSizeInBits();
    DebugLoc DL = getCurDebugLoc();
...
void SelectionDAGBuilder::visitSelect(const User &I) {
  ...
  SelectionDagValue Cond     = getValue(I.getOperand(0));
  SelectionDagValue TrueVal  = getValue(I.getOperand(1));
  SelectionDagValue FalseVal = getValue(I.getOperand(2));
...
void SelectionDAGBuilder::visitTrunc(const User &I) {
  ...
  SelectionDagValue N = getValue(I.getOperand(0));
  EVT DestVT = TLI.getValueType(I.getType());
...
void SelectionDAGBuilder::visitInsertElement(const User &I) {
  SelectionDagValue InVec = getValue(I.getOperand(0));
  SelectionDagValue InVal = getValue(I.getOperand(1));
  SelectionDagValue InIdx = DAG.getNode(ISD::ZERO_EXTEND, getCurDebugLoc(),
                              TLI.getPointerTy(),
...
void SelectionDAGBuilder::visitShuffleVector(const User &I) {
  SelectionDagValue Src1 = getValue(I.getOperand(0));
  SelectionDagValue Src2 = getValue(I.getOperand(1));
  ...
  if (SrcNumElts < MaskNumElts && MaskNumElts % SrcNumElts == 0) {
    ...
    int NumConcat = MaskNumElts / SrcNumElts;
    bool Src1U = Src1.getOpcode() == ISD::UNDEF;
    bool Src2U = Src2.getOpcode() == ISD::UNDEF;
    SelectionDagValue UndefVal = DAG.getUNDEF(SrcVT);
  ...
  if (SrcNumElts > MaskNumElts) {
    ...
    if (RangeUse[0] >= 0 && RangeUse[1] >= 0) {
      ...
      for (int Input = 0; Input < 2; ++Input) {
        SelectionDagValue &Src = Input == 0 ? Src1 : Src2;
        if (RangeUse[Input] == 0)
          Src = DAG.getUNDEF(VT);
        else
  ...
  for (int i = 0; i != MaskNumElts; ++i) {
    int Idx = Mask[i];
    SelectionDagValue Res;

    if (Idx < 0) {
      Res = DAG.getUNDEF(EltVT);
    } else {
      SelectionDagValue &Src = Idx < (int)SrcNumElts ? Src1 : Src2;
...
void SelectionDAGBuilder::visitInsertValue(const InsertValueInst &I) {
  ...
  int LinearIndex = ComputeLinearIndex(AggTy, I.getIndices());
  ...
  SelectionDagValue Agg = getValue(Op0);
  int i = 0;
  ...
  if (NumValValues) {
    SelectionDagValue Val = getValue(Op1);
...
void SelectionDAGBuilder::visitGetElementPtr(const User &I) {
  ...
  for (GetElementPtrInst::const_op_iterator OI = I.op_begin()+1, E = I.op_end();
    ...
    if (StructType *StTy = dyn_cast<StructType>(Ty)) {
      int Field = cast<ConstantInt>(Idx)->getZExtValue();
      if (Field) {
        // N = N + Offset
      ...
      SelectionDagValue IdxN = getValue(Idx);
      ...
      if (ElementSize != 1) {
        if (ElementSize.isPowerOf2()) {
          int Amt = ElementSize.logBase2();
          IdxN = DAG.getNode(ISD::SHL, getCurDebugLoc(),
                             N.getValueType(), IdxN,
                             DAG.getConstant(Amt, IdxN.getValueType()));
        } else {
          SelectionDagValue Scale = DAG.getConstant(ElementSize, TLI.getPointerTy());
...
void SelectionDAGBuilder::visitAlloca(const AllocaInst &I) {
  ...
  int Align =
    std::max((int)TLI.getTargetData()->getPrefTypeAlignment(Ty),
             I.getAlignment());

  SelectionDagValue AllocSize = getValue(I.getArraySize());
  ...
  SelectionDagValue Ops[] = { getRoot(), AllocSize, DAG.getIntPtrConstant(Align) };
  SDVTList VTs = DAG.getVTList(AllocSize.getValueType(), MVT::Other);
  SelectionDagValue DSA = DAG.getNode(ISD::DYNAMIC_STACKALLOC, getCurDebugLoc(),
                            VTs, Ops, 3);
  setValue(&I, DSA);
...
void SelectionDAGBuilder::visitLoad(const LoadInst &I) {
  ...
  SelectionDagValue Ptr = getValue(SV);
  ...
  SmallVector<SDValue, 4> Values(NumValues);
  SmallVector<SDValue, 4> Chains(std::min(int(MaxParallelChains),
                                          NumValues));
  EVT PtrVT = Ptr.getValueType();
  int ChainI = 0;
  for (int i = 0; i != NumValues; ++i, ++ChainI) {
    ...
    if (ChainI == MaxParallelChains) {
      ...
      SelectionDagValue Chain = DAG.getNode(ISD::TokenFactor, getCurDebugLoc(),
                                  MVT::Other, &Chains[0], ChainI);
      Root = Chain;
      ChainI = 0;
    }
    SelectionDagValue A = DAG.getNode(ISD::ADD, getCurDebugLoc(),
                            PtrVT, Ptr,
                            DAG.getConstant(Offsets[i], PtrVT));
    SelectionDagValue L = DAG.getLoad(ValueVTs[i], getCurDebugLoc(), Root,
...
void SelectionDAGBuilder::visitStore(const StoreInst &I) {
  ...
  SelectionDagValue Src = getValue(SrcV);
  SelectionDagValue Ptr = getValue(PtrV);

  SelectionDagValue Root = getRoot();
  SmallVector<SDValue, 4> Chains(std::min(int(MaxParallelChains),
                                          NumValues));
  EVT PtrVT = Ptr.getValueType();
  bool isVolatile = I.isVolatile();
  ...
  for (int i = 0; i != NumValues; ++i, ++ChainI) {
    ...
    SelectionDagValue Add = DAG.getNode(ISD::ADD, getCurDebugLoc(), PtrVT, Ptr,
                              DAG.getConstant(Offsets[i], PtrVT));
    SelectionDagValue St = DAG.getStore(Root, getCurDebugLoc(),
  ...
  SelectionDagValue StoreNode = DAG.getNode(ISD::TokenFactor, getCurDebugLoc(),
                                  MVT::Other, &Chains[0], ChainI);
  ++SDNodeOrder;
...
static SelectionDagValue InsertFenceForAtomic(SDValue Chain, AtomicOrdering Order,
...
void SelectionDAGBuilder::visitAtomicCmpXchg(const AtomicCmpXchgInst &I) {
  ...
  SelectionDagValue InChain = getRoot();
  ...
  SelectionDagValue OutChain = L.getValue(1);
...
void SelectionDAGBuilder::visitTargetIntrinsic(const CallInst &I,
  ...
  for (int i = 0, e = I.getNumArgOperands(); i != e; ++i) {
    SelectionDagValue Op = getValue(I.getArgOperand(i));
    Ops.push_back(Op);
  }
  ...
  if (HasChain) {
    SelectionDagValue Chain = Result.getValue(Result.getNode()->getNumValues()-1);
    if (OnlyLoad)
      PendingLoads.push_back(Chain);
    else
      DAG.setRoot(Chain);
...
static SDValue
GetSignificand(SelectionDAG &DAG, SelectionDagValue Op, DebugLoc dl) {
  SelectionDagValue t1 = DAG.getNode(ISD::AND, dl, MVT::i32, Op,
                           DAG.getConstant(0x007fffff, MVT::i32));
  SelectionDagValue t2 = DAG.getNode(ISD::OR, dl, MVT::i32, t1,
...
/// where Op is the hexidecimal representation of floating point value.
static SDValue
GetExponent(SelectionDAG &DAG, SelectionDagValue Op, const TargetLowering &TLI,
            DebugLoc dl) {
  SelectionDagValue t0 = DAG.getNode(ISD::AND, dl, MVT::i32, Op,
                           DAG.getConstant(0x7f800000, MVT::i32));
  SelectionDagValue t1 = DAG.getNode(ISD::SRL, dl, MVT::i32, t0,
                           DAG.getConstant(23, TLI.getPointerTy()));
  SelectionDagValue t2 = DAG.getNode(ISD::SUB, dl, MVT::i32, t1,
...
static SDValue
getF32Constant(SelectionDAG &DAG, int Flt) {
...
SelectionDAGBuilder::visitExp(const CallInst &I) {
  ...
  if (getValue(I.getArgOperand(0)).getValueType() == MVT::f32 &&
    ...
    SelectionDagValue Op = getValue(I.getArgOperand(0));
    ...
    SelectionDagValue t0 = DAG.getNode(ISD::FMUL, dl, MVT::f32, Op,
                             getF32Constant(DAG, 0x3fb8aa3b));
    SelectionDagValue IntegerPartOfX = DAG.getNode(ISD::FP_TO_SINT, dl, MVT::i32, t0);

    //   FractionalPartOfX = (X * LOG2OFe) - (float)IntegerPartOfX;
    SelectionDagValue t1 = DAG.getNode(ISD::SINT_TO_FP, dl, MVT::f32, IntegerPartOfX);
    SelectionDagValue X = DAG.getNode(ISD::FSUB, dl, MVT::f32, t0, t1);
    ...
    if (LimitFloatPrecision <= 6) {
      ...
      SelectionDagValue t2 = DAG.getNode(ISD::FMUL, dl, MVT::f32, X,
                               getF32Constant(DAG, 0x3e814304));
      SelectionDagValue t3 = DAG.getNode(ISD::FADD, dl, MVT::f32, t2,
                               getF32Constant(DAG, 0x3f3c50c8));
      SelectionDagValue t4 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t3, X);
      SelectionDagValue t5 = DAG.getNode(ISD::FADD, dl, MVT::f32, t4,
                               getF32Constant(DAG, 0x3f7f5e7e));
      SelectionDagValue TwoToFracPartOfX = DAG.getNode(ISD::BITCAST, dl,MVT::i32, t5);

      // Add the exponent into the result in integer domain.
      SelectionDagValue t6 = DAG.getNode(ISD::ADD, dl, MVT::i32,
      ...
      SelectionDagValue t6 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t5, X);
      SelectionDagValue t7 = DAG.getNode(ISD::FADD, dl, MVT::f32, t6,
                               getF32Constant(DAG, 0x3f7ff8fd));
      SelectionDagValue TwoToFracPartOfX = DAG.getNode(ISD::BITCAST, dl,MVT::i32, t7);

      // Add the exponent into the result in integer domain.
      SelectionDagValue t8 = DAG.getNode(ISD::ADD, dl, MVT::i32,
                               TwoToFracPartOfX, IntegerPartOfX);
      ...
      SelectionDagValue t8 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t7, X);
      SelectionDagValue t9 = DAG.getNode(ISD::FADD, dl, MVT::f32, t8,
                               getF32Constant(DAG, 0x3e75fe14));
      SelectionDagValue t10 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t9, X);
      SelectionDagValue t11 = DAG.getNode(ISD::FADD, dl, MVT::f32, t10,
                                getF32Constant(DAG, 0x3f317234));
      SelectionDagValue t12 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t11, X);
      SelectionDagValue t13 = DAG.getNode(ISD::FADD, dl, MVT::f32, t12,
                                getF32Constant(DAG, 0x3f800000));
      SelectionDagValue TwoToFracPartOfX = DAG.getNode(ISD::BITCAST, dl,
                                             MVT::i32, t13);

      // Add the exponent into the result in integer domain.
      SelectionDagValue t14 = DAG.getNode(ISD::ADD, dl, MVT::i32,
                                TwoToFracPartOfX, IntegerPartOfX);
...
SelectionDAGBuilder::visitLog(const CallInst &I) {
  ...
  if (getValue(I.getArgOperand(0)).getValueType() == MVT::f32 &&
      LimitFloatPrecision > 0 && LimitFloatPrecision <= 18) {
    SelectionDagValue Op = getValue(I.getArgOperand(0));
    SelectionDagValue Op1 = DAG.getNode(ISD::BITCAST, dl, MVT::i32, Op);

    // Scale the exponent by log(2) [0.69314718f].
    SelectionDagValue Exp = GetExponent(DAG, Op1, TLI, dl);
    SelectionDagValue LogOfExponent = DAG.getNode(ISD::FMUL, dl, MVT::f32, Exp,
                                        getF32Constant(DAG, 0x3f317218));

    // Get the significand and build it into a floating-point number with
    // exponent of 1.
    SelectionDagValue X = GetSignificand(DAG, Op1, dl);
    ...
    if (LimitFloatPrecision <= 6) {
      ...
      SelectionDagValue t0 = DAG.getNode(ISD::FMUL, dl, MVT::f32, X,
                               getF32Constant(DAG, 0xbe74c456));
      SelectionDagValue t1 = DAG.getNode(ISD::FADD, dl, MVT::f32, t0,
                               getF32Constant(DAG, 0x3fb3a2b1));
      SelectionDagValue t2 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t1, X);
      SelectionDagValue LogOfMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t2,
      ...
      SelectionDagValue t2 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t1, X);
      SelectionDagValue t3 = DAG.getNode(ISD::FSUB, dl, MVT::f32, t2,
                               getF32Constant(DAG, 0x3fbc278b));
      SelectionDagValue t4 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t3, X);
      SelectionDagValue t5 = DAG.getNode(ISD::FADD, dl, MVT::f32, t4,
                               getF32Constant(DAG, 0x40348e95));
      SelectionDagValue t6 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t5, X);
      SelectionDagValue LogOfMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t6,
      ...
      SelectionDagValue t6 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t5, X);
      SelectionDagValue t7 = DAG.getNode(ISD::FSUB, dl, MVT::f32, t6,
                               getF32Constant(DAG, 0x406cfd1c));
      SelectionDagValue t8 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t7, X);
      SelectionDagValue t9 = DAG.getNode(ISD::FADD, dl, MVT::f32, t8,
                               getF32Constant(DAG, 0x408797cb));
      SelectionDagValue t10 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t9, X);
      SelectionDagValue LogOfMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t10,
...
SelectionDAGBuilder::visitLog2(const CallInst &I) {
  ...
  if (getValue(I.getArgOperand(0)).getValueType() == MVT::f32 &&
    ...
    // Get the exponent.
    SelectionDagValue LogOfExponent = GetExponent(DAG, Op1, TLI, dl);
    ...
    if (LimitFloatPrecision <= 6) {
      ...
      SelectionDagValue t2 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t1, X);
      SelectionDagValue Log2ofMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t2,
      ...
      SelectionDagValue t6 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t5, X);
      SelectionDagValue Log2ofMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t6,
      ...
      SelectionDagValue t10 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t9, X);
      SelectionDagValue Log2ofMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t10,
...
SelectionDAGBuilder::visitLog10(const CallInst &I) {
  ...
  if (getValue(I.getArgOperand(0)).getValueType() == MVT::f32 &&
    ...
    if (LimitFloatPrecision <= 6) {
      ...
      SelectionDagValue t2 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t1, X);
      SelectionDagValue Log10ofMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t2,
      ...
      SelectionDagValue t1 = DAG.getNode(ISD::FSUB, dl, MVT::f32, t0,
                               getF32Constant(DAG, 0x3ea21fb2));
      SelectionDagValue t2 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t1, X);
      SelectionDagValue t3 = DAG.getNode(ISD::FADD, dl, MVT::f32, t2,
                               getF32Constant(DAG, 0x3f6ae232));
      SelectionDagValue t4 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t3, X);
      SelectionDagValue Log10ofMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t4,
      ...
      SelectionDagValue t4 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t3, X);
      SelectionDagValue t5 = DAG.getNode(ISD::FSUB, dl, MVT::f32, t4,
                               getF32Constant(DAG, 0x3f88d192));
      SelectionDagValue t6 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t5, X);
      SelectionDagValue t7 = DAG.getNode(ISD::FADD, dl, MVT::f32, t6,
                               getF32Constant(DAG, 0x3fc4316c));
      SelectionDagValue t8 = DAG.getNode(ISD::FMUL, dl, MVT::f32, t7, X);
      SelectionDagValue Log10ofMantissa = DAG.getNode(ISD::FSUB, dl, MVT::f32, t8,
...
SelectionDAGBuilder::visitExp2(const CallInst &I) {
  ...
  if (getValue(I.getArgOperand(0)).getValueType() == MVT::f32 &&
    ...
    SelectionDagValue IntegerPartOfX = DAG.getNode(ISD::FP_TO_SINT, dl, MVT::i32, Op);

    //   FractionalPartOfX = x - (float)IntegerPartOfX;
    SelectionDagValue t1 = DAG.getNode(ISD::SINT_TO_FP, dl, MVT::f32, IntegerPartOfX);
    SelectionDagValue X = DAG.getNode(ISD::FSUB, dl, MVT::f32, Op, t1);
    ...
    if (LimitFloatPrecision <= 6) {
      ...
      SelectionDagValue t5 = DAG.getNode(ISD::FADD, dl, MVT::f32, t4,
                               getF32Constant(DAG, 0x3f7f5e7e));
      SelectionDagValue t6 = DAG.getNode(ISD::BITCAST, dl, MVT::i32, t5);
      SelectionDagValue TwoToFractionalPartOfX =
      ...
      SelectionDagValue t7 = DAG.getNode(ISD::FADD, dl, MVT::f32, t6,
                               getF32Constant(DAG, 0x3f7ff8fd));
      SelectionDagValue t8 = DAG.getNode(ISD::BITCAST, dl, MVT::i32, t7);
      SelectionDagValue TwoToFractionalPartOfX =
      ...
      SelectionDagValue t13 = DAG.getNode(ISD::FADD, dl, MVT::f32, t12,
                                getF32Constant(DAG, 0x3f800000));
      SelectionDagValue t14 = DAG.getNode(ISD::BITCAST, dl, MVT::i32, t13);
      SelectionDagValue TwoToFractionalPartOfX =
...
SelectionDAGBuilder::visitPow(const CallInst &I) {
  ...
  if (IsExp10 && LimitFloatPrecision > 0 && LimitFloatPrecision <= 18) {
    SelectionDagValue Op = getValue(I.getArgOperand(1));
...
static SelectionDagValue ExpandPowI(DebugLoc DL, SelectionDagValue LHS, SelectionDagValue RHS,
                          SelectionDAG &DAG) {
  ...
  if (ConstantSDNode *RHSC = dyn_cast<ConstantSDNode>(RHS)) {
    ...
    if (!F->hasFnAttr(Attribute::OptimizeForSize) ||
      ...
      SelectionDagValue Res;  // Logically starts equal to 1.0
      SelectionDagValue CurSquare = LHS;
      while (Val) {
        if (Val & 1) {
          if (Res.getNode())
...
// argument.
static int getTruncatedArgReg(const SelectionDagValue &N) {
  if (N.getOpcode() != ISD::TRUNCATE)
    return 0;

  const SelectionDagValue &Ext = N.getOperand(0);
  if (Ext.getOpcode() == ISD::AssertZext || Ext.getOpcode() == ISD::AssertSext){
    const SelectionDagValue &CFR = Ext.getOperand(0);
...
SelectionDAGBuilder::EmitFuncArgumentDbgValue(const Value *V, MDNode *Variable,
  ...
  if (!Reg && N.getNode()) {
    ...
    if (Reg && TargetRegisterInfo::isVirtualRegister(Reg)) {
      ...
      int PR = RegInfo.getLiveInPhysReg(Reg);
      if (PR)
        Reg = PR;
    }
  }
...
const char *
SelectionDAGBuilder::visitIntrinsicCall(const CallInst &I, int Intrinsic) {
  DebugLoc dl = getCurDebugLoc();
  SelectionDagValue Res;
  ...
  case Intrinsic::memcpy: {
    ...
    SelectionDagValue Op1 = getValue(I.getArgOperand(0));
    SelectionDagValue Op2 = getValue(I.getArgOperand(1));
    SelectionDagValue Op3 = getValue(I.getArgOperand(2));
    int Align = cast<ConstantInt>(I.getArgOperand(3))->getZExtValue();
  ...
  case Intrinsic::dbg_declare: {
    ...
    SelectionDagValue &N = NodeMap[Address];
  ...
  case Intrinsic::eh_typeid_for: {
    ...
    int TypeID = DAG.getMachineFunction().getMMI().getTypeIDFor(GV);
  ...
  case Intrinsic::eh_dwarf_cfa: {
    SelectionDagValue CfaArg = DAG.getSExtOrTrunc(getValue(I.getArgOperand(0)), dl,
                                        TLI.getPointerTy());
    SelectionDagValue Offset = DAG.getNode(ISD::ADD, dl,
                                 TLI.getPointerTy(),
                                 DAG.getNode(ISD::FRAME_TO_ARGS_OFFSET, dl,
                                             TLI.getPointerTy()),
                                 CfaArg);
    SelectionDagValue FA = DAG.getNode(ISD::FRAMEADDR, dl,
                             TLI.getPointerTy(),
  ...
  case Intrinsic::eh_sjlj_setjmp: {
    ...
    SelectionDagValue Op = DAG.getNode(ISD::EH_SJLJ_SETJMP, dl,
  ...
  case Intrinsic::x86_mmx_psrai_d: {
    SelectionDagValue ShAmt = getValue(I.getArgOperand(1));
  ...
  case Intrinsic::pcmarker: {
    SelectionDagValue Tmp = getValue(I.getArgOperand(0));
    DAG.setRoot(DAG.getNode(ISD::PCMARKER, dl, MVT::Other, getRoot(), Tmp));
    return 0;
  }
  case Intrinsic::readcyclecounter: {
    SelectionDagValue Op = getRoot();
  ...
  case Intrinsic::bswap:
    ...
    return 0;
  case Intrinsic::cttz: {
    SelectionDagValue Arg = getValue(I.getArgOperand(0));
  ...
  case Intrinsic::stackprotector: {
    ...
    SelectionDagValue Src = getValue(I.getArgOperand(0));   // The guard's value.
    AllocaInst *Slot = cast<AllocaInst>(I.getArgOperand(1));

    int FI = FuncInfo.StaticAllocaMap[Slot];
    MFI->setStackProtectorIndex(FI);

    SelectionDagValue FIN = DAG.getFrameIndex(FI, PtrTy);
  ...
  case Intrinsic::objectsize: {
    ...
    SelectionDagValue Arg = getValue(I.getCalledValue());
    EVT Ty = Arg.getValueType();
  ...
  case Intrinsic::prefetch: {
    SelectionDagValue Ops[5];
    int rw = cast<ConstantInt>(I.getArgOperand(1))->getZExtValue();
    Ops[0] = getRoot();
    Ops[1] = getValue(I.getArgOperand(0));
    Ops[2] = getValue(I.getArgOperand(1));
    Ops[3] = getValue(I.getArgOperand(2));
...
void SelectionDAGBuilder::LowerCallTo(ImmutableCallSite CS, SelectionDagValue Callee,
                                      bool isTailCall,
  ...
  SelectionDagValue DemoteStackSlot;
  int DemoteStackIdx = -100;

  if (!CanLowerReturn) {
    uint64_t TySize = TLI.getTargetData()->getTypeAllocSize(
                      FTy->getReturnType());
    int Align  = TLI.getTargetData()->getPrefTypeAlignment(
                      FTy->getReturnType());
    MachineFunction &MF = DAG.getMachineFunction();
  ...
  for (ImmutableCallSite::arg_iterator i = CS.arg_begin(), e = CS.arg_end();
    ...
    SelectionDagValue ArgNode = getValue(V);
    Entry.Node = ArgNode; Entry.Ty = V->getType();

    int attrInd = i - CS.arg_begin() + 1;
  ...
  if (Result.first.getNode()) {
    ...
    EVT PtrVT = PVTs[0];
    int NumValues = Outs.size();
    SmallVector<SDValue, 4> Values(NumValues);
    SmallVector<SDValue, 4> Chains(NumValues);

    for (int i = 0; i < NumValues; ++i) {
      SelectionDagValue Add = DAG.getNode(ISD::ADD, getCurDebugLoc(), PtrVT,
                                DemoteStackSlot,
                                DAG.getConstant(Offsets[i], PtrVT));
      SelectionDagValue L = DAG.getLoad(Outs[i].VT, getCurDebugLoc(), Result.second,
                              Add,
    ...
    int CurReg = 0;
    for (int I = 0, E = RetTys.size(); I != E; ++I) {
      EVT VT = RetTys[I];
      EVT RegisterVT = TLI.getRegisterType(RetTy->getContext(), VT);
      int NumRegs = TLI.getNumRegisters(RetTy->getContext(), VT);

      SelectionDagValue ReturnValue =
...
static SelectionDagValue getMemCmpLoad(const Value *PtrVal, MVT LoadVT,
                             Type *LoadTy,
  ...
  SelectionDagValue Ptr = Builder.getValue(PtrVal);
  SelectionDagValue LoadVal = Builder.DAG.getLoad(LoadVT, Builder.getCurDebugLoc(), Root,
...
bool SelectionDAGBuilder::visitMemCmpCall(const CallInst &I) {
  ...
  if (Size && IsOnlyUsedInZeroEqualityComparison(&I)) {
    ...
    if (ActuallyDoIt) {
      SelectionDagValue LHSVal = getMemCmpLoad(LHS, LoadVT, LoadTy, *this);
      SelectionDagValue RHSVal = getMemCmpLoad(RHS, LoadVT, LoadTy, *this);

      SelectionDagValue Res = DAG.getSetCC(getCurDebugLoc(), MVT::i1, LHSVal, RHSVal,
                                 ISD::SETNE);
  ...
  if (Function *F = I.getCalledFunction()) {
    if (F->isDeclaration()) {
      if (const TargetIntrinsicInfo *II = TM.getIntrinsicInfo()) {
        if (int IID = II->getIntrinsicID(F)) {
          RenameFn = visitIntrinsicCall(I, IID);
          if (!RenameFn)
            return;
        }
      }
      if (int IID = F->getIntrinsicID()) {
        RenameFn = visitIntrinsicCall(I, IID);
        if (!RenameFn)
          return;
      }
    ...
    if (!F->hasLocalLinkage() && F->hasName()) {
      ...
      if ((LibInfo->has(LibFunc::copysign) && Name == "copysign") ||
        ...
        if (I.getNumArgOperands() == 2 &&   // Basic sanity checks.
          ...
          SelectionDagValue LHS = getValue(I.getArgOperand(0));
          SelectionDagValue RHS = getValue(I.getArgOperand(1));
...
public:
  ...
  EVT getCallOperandValEVT(LLVMContext &Context,
    ...
    if (!OpTy->isSingleValueType() && OpTy->isSized()) {
      int BitSize = TD->getTypeSizeInBits(OpTy);
      switch (BitSize) {
      default: break;
      case 1:
      case 8:
...
static void GetRegistersForValue(SelectionDAG &DAG,
  ...
  std::pair<int, const TargetRegisterClass*> PhysReg =
  ...
  // assign it now.
  if (int AssignedReg = PhysReg.first) {
...
void SelectionDAGBuilder::visitInlineAsm(ImmutableCallSite CS) {
  ...
  int ArgNo = 0;   // ArgNo - The argument of the CallInst.
  int ResNo = 0;   // ResNo - The result number of the next output.
  for (int i = 0, e = TargetConstraints.size(); i != e; ++i) {
    ...
    if (OpInfo.isIndirect)
      ...
      for (int j = 0, ee = OpInfo.Codes.size(); j != ee; ++j) {
  ...
  for (int i = 0, e = ConstraintOperands.size(); i != e; ++i) {
	...
	std::pair<int, const TargetRegisterClass*> MatchRC =
	  TLI.getRegForInlineAsmConstraint(OpInfo.ConstraintCode,
                                           OpInfo.ConstraintVT);
	std::pair<int, const TargetRegisterClass*> InputRC =
    ...
    if (OpInfo.ConstraintType == TargetLowering::C_Memory &&
      ...
      if (isa<ConstantFP>(OpVal) || isa<ConstantInt>(OpVal) ||
        ...
        uint64_t TySize = TLI.getTargetData()->getTypeAllocSize(Ty);
        int Align  = TLI.getTargetData()->getPrefTypeAlignment(Ty);
        MachineFunction &MF = DAG.getMachineFunction();
        int SSFI = MF.getFrameInfo()->CreateStackObject(TySize, Align, false);
        SelectionDagValue StackSlot = DAG.getFrameIndex(SSFI, TLI.getPointerTy());
  ...
  for (int i = 0, e = ConstraintOperands.size(); i != e; ++i) {
    ...
    case InlineAsm::isOutput: {
      if (OpInfo.ConstraintType != TargetLowering::C_RegisterClass &&
        ...
        int OpFlags = InlineAsm::getFlagWord(InlineAsm::Kind_Mem, 1);
    ...
    case InlineAsm::isInput: {
      SelectionDagValue InOperandVal = OpInfo.CallOperand;

      if (OpInfo.isMatchingInputConstraint()) {   // Matching constraint?
        // If this is required to match an output register we have already set,
        // just use its register.
        int OperandNo = OpInfo.getMatchedOperand();

        // Scan until we find the definition we already emitted of this operand.
        // When we find it, create a RegsForValue operand.
        int CurOp = InlineAsm::Op_FirstOperand;
        ...
        if (InlineAsm::isRegDefKind(OpFlag) ||
          ...
          for (int i = 0, e = InlineAsm::getNumOperandRegisters(OpFlag);
               i != e; ++i)
      ...
      if (OpInfo.ConstraintType == TargetLowering::C_Memory) {
        ...
        // Add information to the INLINEASM node to know about this input.
        int ResOpType = InlineAsm::getFlagWord(InlineAsm::Kind_Mem, 1);
        AsmNodeOperands.push_back(DAG.getTargetConstant(ResOpType,
                                                        TLI.getPointerTy()));
  ...
  if (!RetValRegs.Regs.empty()) {
    SelectionDagValue Val = RetValRegs.getCopyFromRegs(DAG, FuncInfo, getCurDebugLoc(),
                                             Chain, &Flag);
  ...
  // physregs.
  for (int i = 0, e = IndirectStoresToEmit.size(); i != e; ++i) {
    RegsForValue &OutRegs = IndirectStoresToEmit[i].first;
    const Value *Ptr = IndirectStoresToEmit[i].second;
    SelectionDagValue OutVal = OutRegs.getCopyFromRegs(DAG, FuncInfo, getCurDebugLoc(),
                                             Chain, &Flag);
  ...
  for (int i = 0, e = StoresToEmit.size(); i != e; ++i) {
    SelectionDagValue Val = DAG.getStore(Chain, getCurDebugLoc(),
...
void SelectionDAGBuilder::visitVAArg(const VAArgInst &I) {
  ...
  SelectionDagValue V = DAG.getVAArg(TLI.getValueType(I.getType()), getCurDebugLoc(),
...
TargetLowering::LowerCallTo(SDValue Chain, Type *RetTy,
  ...
  for (int i = 0, e = Args.size(); i != e; ++i) {
    ...
    for (int Value = 0, NumValues = ValueVTs.size();
         Value != NumValues; ++Value) {
      EVT VT = ValueVTs[Value];
      Type *ArgTy = VT.getTypeForEVT(RetTy->getContext());
      SelectionDagValue Op = SDValue(Args[i].Node.getNode(),
                           Args[i].Node.getResNo() + Value);
      ISD::ArgFlagsTy Flags;
      int OriginalAlignment =
      ...
      int NumParts = getNumRegisters(RetTy->getContext(), VT);
      SmallVector<SDValue, 4> Parts(NumParts);
      ISD::NodeType ExtendKind = ISD::ANY_EXTEND;
  ...
  for (int I = 0, E = RetTys.size(); I != E; ++I) {
    ...
    int NumRegs = getNumRegisters(RetTy->getContext(), VT);
    for (int i = 0; i != NumRegs; ++i) {
  ...
  DEBUG(for (int i = 0, e = Ins.size(); i != e; ++i) {
  ...
  SelectionDagValue Res = DAG.getNode(ISD::MERGE_VALUES, dl,
                            DAG.getVTList(&RetTys[0], RetTys.size()),
                            &ReturnValues[0], ReturnValues.size());
...
void TargetLowering::LowerOperationWrapper(SDNode *N,
  ...
  SelectionDagValue Res = LowerOperation(SDValue(N, 0), DAG);
  if (Res.getNode())
...
void
SelectionDAGBuilder::CopyValueToVirtualRegister(const Value *V, int Reg) {
  SelectionDagValue Op = getNonRegisterValue(V);
...
void SelectionDAGISel::LowerArguments(const BasicBlock *LLVMBB) {
  ...
  SelectionDagValue NewRoot = TLI.LowerFormalArguments(DAG.getRoot(), F.getCallingConv(),
                                             F.isVarArg(), Ins,
                                             dl, DAG, InVals);
  ...
  if (!FuncInfo->CanLowerReturn) {
    ...
    ISD::NodeType AssertOp = ISD::DELETED_NODE;
    SelectionDagValue ArgValue = getCopyFromParts(DAG, dl, &InVals[0], 1,
                                        RegVT, VT, AssertOp);

    MachineFunction& MF = SDB->DAG.getMachineFunction();
    MachineRegisterInfo& RegInfo = MF.getRegInfo();
    int SRetReg = RegInfo.createVirtualRegister(TLI.getRegClassFor(RegVT));
  ...
  for (Function::const_arg_iterator I = F.arg_begin(), E = F.arg_end(); I != E;
    ...
    SelectionDagValue Res = DAG.getMergeValues(&ArgValues[0], NumValues,
                                     SDB->getCurDebugLoc());
    ...
    if (!TM.Options.EnableFastISel && Res.getOpcode() == ISD::CopyFromReg) {
      ...
      // uses with vregs.
      int Reg = cast<RegisterSDNode>(Res.getOperand(1))->getReg();
...
SelectionDAGBuilder::HandlePHINodesInSuccessorBlocks(const BasicBlock *LLVMBB) {
  ...
  // from this block.
  for (int succ = 0, e = TI->getNumSuccessors(); succ != e; ++succ) {
    ...
    for (BasicBlock::const_iterator I = SuccBB->begin();
      ...
      if (const Constant *C = dyn_cast<Constant>(PHIOp)) {
        ...
        DenseMap<const Value *, int>::iterator I =
      ...
      for (int vti = 0, vte = ValueVTs.size(); vti != vte; ++vti) {
        EVT VT = ValueVTs[vti];
        int NumRegisters = TLI.getNumRegisters(*DAG.getContext(), VT);
        for (int i = 0, e = NumRegisters; i != e; ++i)
