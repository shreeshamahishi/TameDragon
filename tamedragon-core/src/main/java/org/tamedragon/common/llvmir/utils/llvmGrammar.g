...
binary_instruction : num=result EQUAL oprstr=BIN_OPR_STR flag=BIN_OPR_FLAG? typ=data_type o1=result COMMA o2=result						
				   {				
...
load : res=result e0=EQUAL 'load' pointee=data_type COMMA ptr=data_type loc=result (COMMA ALIGN NUMBER)?
	 {							
	 	setLoadInstr($pointee.text, $res.text,$loc.text,$ptr.text, $e0.line, 0);
	 };
...
getelementptr_rhs[String name, int line] : 'getelementptr'inb='inbounds'? START_PARANTHESIS? pointee=data_type COMMA ptr = ptr_type 
											element=element_name COMMA off= offset END_PARANTHESIS?
				    					 {	    		
											setGetElementPtr($pointee.text, name,$element.text,$ptr.text, $off.text,$inb.text, line, 0);
				    					 };
