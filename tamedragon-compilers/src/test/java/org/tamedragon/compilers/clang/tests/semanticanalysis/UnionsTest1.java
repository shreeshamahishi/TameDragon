...
public class UnionsTest1 extends TestInitializer {
	...
	public void setUp(){
		...
		environments.reset();
		sourceFilePath ="CSrc/Semantic/UnionsTest1.c"; 
	...
	public void test1() {      		
		...
		assertTrue(errorHandler.getNumErrors() == 8);
		...
		try{
			while(iter.hasNext()){
				...
				if(count == 1){
					...
					assertTrue(line == 30); 		
					assertTrue(msg.equals(ErrorHandler.W_EXCESS_INITIALIZERS_IN_UNION_INIT ));				
				}
				else if(count == 6){
					assertTrue(line == 32); 
					assertTrue(msg.equals(ErrorHandler.W_EXCESS_INITIALIZERS_IN_UNION_INIT));						
