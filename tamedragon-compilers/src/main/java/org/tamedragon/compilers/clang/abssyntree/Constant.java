...
public class Constant  extends Absyn {
	...
	public static final int FLOAT_CONST = 2;
	...
	public String getFloatingConstant() {
		return floatingConstant;
	}
	public void setFloatingConstant(String floatingConstant) {
		type = FLOAT_CONST;
		this.floatingConstant = floatingConstant;
	}
	...
	/**
	 * Will return true if the constant is to be regarded as a long int. This can 
	 * happen if: the constant ends with "l" or "L" (explicity declared). Also, if the constant 
	 * exceeds the max possible for a 32-bit integer.
	 * @return
	 */
	public boolean isDeclaredAsLong(){
		if(type != INT_CONST)
			return false;
		
		// If the constant has been explicitly set at long
		if(intConstant.endsWith("L") || intConstant.endsWith("l")  ){
			return true;
		}
		
		
		// Or if it exceeds 32-bit integer
		if(intConstant.endsWith("L") || intConstant.endsWith("l")  ){
			return true;
		}
		
		if(intConstant.length() > Constant.MAX_SIGNED_LONG_CONST_LENGTH){
			return true;
		}		
		return false;		
	}
	...
	public static boolean canBeFloat(String floatingConstant){
		...
		// If the Float is in Hexadecimal form
		if(floatingConstant.matches("(0[x|X])?([0-9]|[A-F]|[a-f])+")){
			if(floatingConstant.startsWith("0x") || floatingConstant.startsWith("0X")){
				long doubleAsLong2 = Long.decode(floatingConstant);
				floatingConstant  = new Double(Double.longBitsToDouble(doubleAsLong2)).toString();
			}
			else{
				long doubleAsLong2 = Long.decode("0x" + floatingConstant);
				floatingConstant  = new Double(Double.longBitsToDouble(doubleAsLong2)).toString();
			}
		}
