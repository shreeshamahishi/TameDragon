...
public class EqualityExpr  extends Absyn {
	...
	public int evaluate(){
		int val = relationalExpr.evaluate();
		
		if(equalityExpr != null){
			int eeVal = equalityExpr.evaluate();
			
			if((val == eeVal) && (operator == EQUALS))
				return 1;
			else if((val != eeVal) && (operator == NOT_EQUALS))
				return 1;
			else 
				return 0;
		}		
